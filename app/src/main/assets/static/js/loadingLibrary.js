(function($) {
// Some functions copied from CDM project for the loading screen
var styling_data = '<style type="text/css">\
  #loading_screen{\
    position: fixed;\
    top: 0;\
    left: 0;\
...
function check_init(){
  if (!$("#loading_screen").length){
    // if the loading screen div is not present on the page, inject our styling and divs into it
    $("head").children().last().after(styling_data);
    $("body").children().first().before(loading_data);
    $.myloadingscreen = $("#loading_screen");
  }
}
$.showLoading = function(){
  // Increment the number of loading screens we show to provide nesting
  check_init();
  // now, we are ready to display
  $.myloadingscreen.show();
  $.loadingcount += 1;
}

$.hideLoading = function(){
  // decrement the number of times showLoading has been called. If it goes to zero, we have recursively returned back to home - remove the loading gif animation
  check_init();
  if($.loadingcount)
    $.loadingcount -= 1;
  if(!$.loadingcount){
    $.writeLoading("");
    $.myloadingscreen.hide();
  }
}

$.writeLoading = function(text_string){
  check_init();
  $.myloadingscreen.find("#loading_text").html(text_string);
}
